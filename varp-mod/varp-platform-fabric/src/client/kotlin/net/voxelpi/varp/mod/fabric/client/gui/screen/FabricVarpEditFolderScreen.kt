...
class FabricVarpEditFolderScreen(
    ...
    override fun build(rootComponent: FlowLayout) {
        ...
        val idInput = Components.textBox(Sizing.fill(60), folder.id).apply {
            setMaxLength(256)
            setEditable(false)
            margins(Insets.vertical(2))
        }

        val nameInput = Components.textBox(Sizing.fill(60), MiniMessage.miniMessage().serialize(folder.name)).apply {
            setMaxLength(1024)
            margins(Insets.vertical(2))
        }
        ...
        menuPlane.child(
            Containers.horizontalFlow(Sizing.fill(95), Sizing.fixed(20)).apply {
                child(
                    Components.texture(Identifier.of("varp:textures/gui/edit_folder.png"), 0, 0, 16, 16, 16, 16).apply {
                        margins(Insets.of(0, 0, 0, 8))
                    }
                )
                child(
                    Components.label(Text.translatable("gui.varp.edit_folder.message")).apply {
            ...
            Containers.verticalFlow(Sizing.fill(100), Sizing.content()).apply {
                child(
                    Containers.horizontalFlow(Sizing.content(), Sizing.content()).apply {
                        child(
                            Components.label(Text.translatable("gui.varp.edit_folder.id")).apply {
                ...
                child(
                    Containers.horizontalFlow(Sizing.content(), Sizing.content()).apply {
                        child(
                            Components.label(Text.translatable("gui.varp.edit_folder.name")).apply {
                                horizontalTextAlignment(HorizontalAlignment.RIGHT)
                                verticalTextAlignment(VerticalAlignment.CENTER)
                                sizing(Sizing.fill(25), Sizing.fixed(24))
                                margins(Insets.right(8))
                ...
                child(
                    Containers.horizontalFlow(Sizing.content(), Sizing.content()).apply {
                        child(
                            Components.label(Text.translatable("gui.varp.edit_folder.parent")).apply {
                                horizontalTextAlignment(HorizontalAlignment.RIGHT)
                                verticalTextAlignment(VerticalAlignment.CENTER)
                                sizing(Sizing.fill(25), Sizing.fixed(24))
                                margins(Insets.right(8))
                        ...
                        child(
                            Components.button(Text.translatable("gui.varp.edit_folder.select_parent")) {}.apply {
        ...
        menuPlane.child(
            Containers.horizontalFlow(Sizing.fill(100), Sizing.content()).apply {
                child(
                    Components.button(Text.translatable("gui.varp.edit_folder.cancel")) {
                ...
                child(
                    Components.button(Text.translatable("gui.varp.edit_folder.confirm")) {
                        val id = idInput.text
