...
class ClientStorage(
    ...
    private val openServerRepositories = mutableMapOf<String, StorageHandle>()
    private val serverRepositoryState = mutableMapOf<String, TreeState>()

    override suspend fun open(config: ClientStorageConfig): Result<StorageHandle> = runCatching {
        if (config.repositoryId in openServerRepositories) {
            throw IllegalStateException("Client storage for repository '${config.repositoryId}' is already open")
        }

        val handle = StorageHandle.Simple()
        openServerRepositories[config.repositoryId] = handle
        return@runCatching handle
    }

    override suspend fun close(config: ClientStorageConfig, handle: StorageHandle): Result<Unit> = runCatching {
    ...
    override suspend fun createFolder(config: ClientStorageConfig, handle: StorageHandle, path: FolderPath, state: FolderState): Result<Unit> {
    ...
    override suspend fun deleteWarp(config: ClientStorageConfig, handle: StorageHandle, path: WarpPath): Result<Unit> {
        clientNetworkHandler.sendServerboundPacket(VarpServerboundDeleteWarpPacket(path))
        return Result.success(Unit)
    }

    override suspend fun deleteFolder(config: ClientStorageConfig, handle: StorageHandle, path: FolderPath): Result<Unit> {
        clientNetworkHandler.sendServerboundPacket(VarpServerboundDeleteFolderPacket(path))
        return Result.success(Unit)
    }

    override suspend fun moveWarp(config: ClientStorageConfig, handle: StorageHandle, src: WarpPath, dst: WarpPath): Result<Unit> {
        clientNetworkHandler.sendServerboundPacket(VarpServerboundModifyWarpPathPacket(src, dst))
        return Result.success(Unit)
    }

    override suspend fun moveFolder(config: ClientStorageConfig, handle: StorageHandle, src: FolderPath, dst: FolderPath): Result<Unit> {
    ...
    internal fun reset() {
        openServerRepositories.clear()
        serverRepositoryState.clear()
    }
}
