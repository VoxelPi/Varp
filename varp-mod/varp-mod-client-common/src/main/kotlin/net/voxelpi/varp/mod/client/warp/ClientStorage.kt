...
class ClientStorage(
    ...
    override suspend fun open(config: Unit): Result<Unit> {
        TODO("Not yet implemented")
    }

    override suspend fun close(config: Unit, handle: Unit): Result<Unit> {
        TODO("Not yet implemented")
    }

    override suspend fun loadContent(config: Unit, handle: Unit): Result<TreeState> {
    ...
    override suspend fun createWarp(config: Unit, handle: Unit, path: WarpPath, state: WarpState): Result<Unit> {
        clientNetworkHandler.sendServerboundPacket(VarpServerboundCreateWarpPacket(path, state))
        return Result.success(Unit)
    }

    override suspend fun createFolder(config: Unit, handle: Unit, path: FolderPath, state: FolderState): Result<Unit> {
        clientNetworkHandler.sendServerboundPacket(VarpServerboundCreateFolderPacket(path, state))
        return Result.success(Unit)
    }

    override suspend fun saveWarp(config: Unit, handle: Unit, path: WarpPath, state: WarpState): Result<Unit> {
        clientNetworkHandler.sendServerboundPacket(VarpServerboundModifyWarpStatePacket(path, state))
        return Result.success(Unit)
    }

    override suspend fun saveFolder(config: Unit, handle: Unit, path: FolderPath, state: FolderState): Result<Unit> {
        clientNetworkHandler.sendServerboundPacket(VarpServerboundModifyFolderStatePacket(path, state))
        return Result.success(Unit)
    }

    override suspend fun saveRoot(config: Unit, handle: Unit, state: FolderState): Result<Unit> {
        clientNetworkHandler.sendServerboundPacket(VarpServerboundModifyRootStatePacket(state))
        return Result.success(Unit)
    }

    override suspend fun deleteWarp(config: Unit, handle: Unit, path: WarpPath): Result<Unit> {
        clientNetworkHandler.sendServerboundPacket(VarpServerboundDeleteWarpPacket(path))
        return Result.success(Unit)
    }

    override suspend fun deleteFolder(config: Unit, handle: Unit, path: FolderPath): Result<Unit> {
        clientNetworkHandler.sendServerboundPacket(VarpServerboundDeleteFolderPacket(path))
        return Result.success(Unit)
    }

    override suspend fun moveWarp(config: Unit, handle: Unit, src: WarpPath, dst: WarpPath): Result<Unit> {
        clientNetworkHandler.sendServerboundPacket(VarpServerboundModifyWarpPathPacket(src, dst))
        return Result.success(Unit)
    }

    override suspend fun moveFolder(config: Unit, handle: Unit, src: FolderPath, dst: FolderPath): Result<Unit> {
